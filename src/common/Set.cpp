...
const std::string default_dir = "/home/cscdev/Ben/pats/tmp/";
const std::string default_ps_output_file = "/home/cscdev/Ben/pattern_studies/paramScan.log";
...
bool Set::AddComparatorHit(Hit hit_in)
{
     ComparatorHit.push_back(hit_in);
     return true;
}
...
{
	if(not (i >=0 && i < CSC.size())){ return false; }
	CSC.erase(CSC.begin() + i);
	return true;
}
bool Set::RemoveComparatorHit(int i)
{
	if(not (i >=0 && i < ComparatorHit.size())){ return false; }
	ComparatorHit.erase(ComparatorHit.begin() + i);
	return true;
}
bool Set::RemoveGEM(int i)
{
	if(not (i>=0 && i < GEM.size())){ return false; }
...
bool Set::WritePatterns(std::string opt_path)
{
	...
	if(opt_path.empty()){
        ...
        if (CSC.size() == 0 and ComparatorHit.size() > 0 )
	return (WritePat(full_path, ComparatorHit)) && (WritePat(full_path, GEM));
        else
...
bool Set::LoadEmuBoard(std::string opt_path)
{
	...
	std::cout << " send on cmslab1, path "<< opt_path << " tmb type "<< Tmb_type << std::endl;
	...
	for (int i = 0; i < CSCConstants::NUM_DCFEBS; i++)
        {
		    ...
		    std::cout << "==== Error!!====, Failed to open File: " << ss.str() << std::endl;
	...
	for (unsigned int i=0; i < pat_files.size(); i++)
	{
		std::cout << "pageid = " << patFile_to_pageID[i] <<" pat file "<< pat_files[i]<< std::endl;
...
void Set::ViewContents(std::ostream& oss)
{
  oss << "******************************************************\n"
            << "                   Muon Tracks in Set\n"
            << "******************************************************\n";
  for(unsigned int i=0; i < CSC.size(); i++){
    oss << '\n';
...
void Set::SaveResults(std::string opt_path)
{
	...
	//ofs << "\nCSC\nbx\ths\tly\t\t{ (bx,hs,ly) }\n";
	//for(int i=0; i < set.ComparatorHit.size(); i++){
	//	ofs << ComparatorHit[i] << std::endl;
	//}
...
std::ostream& operator<<(std::ostream& oss, const Set& set){
	...
	for(unsigned int i=0; i < set.CSC.size(); i++){
		oss << "<C " << set.CSC[i] << ">";	//	C ==> clct
	}
	for(unsigned int i=0; i < set.ComparatorHit.size(); i++){
		oss << "<H " << set.ComparatorHit[i] << ">";	//	H ==> Hit
	}
	for(unsigned int i=0; i < set.GEM.size(); i++){		//	G ==> gem cluster
		oss << "<G " << set.GEM[i] << ">";
	}
	for(unsigned int i=0; i < set.Results.size(); i++){	//	R ==> Expr. Results
